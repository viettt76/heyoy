...
function MovieListLayout() {
    ...
    useEffect(() => {
        if (genresId && genres) {
            const fetchMovieByGenres = async () => {
                try {
                    const res = await services.getMovieByGenres(genresId, page);
            ...
            const fetchApiMoviesPopular = async () => {
                try {
                    ...
                    switch (title) {
                        case 'Popular Movies':
                            res = await services.moviesPopularService(page);
                            break;
                        case 'Now Playing Movies':
                            res = await services.moviesNowPlayingService(page);
                            break;
                        case 'Upcoming Movies':
                            res = await services.moviesUpcomingService(page);
                            break;
                        case 'Top Rated Movies':
                            res = await services.moviesTopRatedService(page);
                            break;
                        default:
                            res = await services.moviesPopularService(page);
                    }
    ...
    const handlePageClick = (e) => {
        if (genres && genresId) {
            navigate(`/movie/list/genres/${genresId}&page/${e.selected + 1}`, { state: { title } });
        } else {
